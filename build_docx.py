...
body.append(para("At the TOP of the Transactions tab (rows 4-20) is an auto-posted Interest ledger - one line per "
                 "account, read-only, generated by formula (not typed). It is kept above the log so the log can grow "
                 "freely, and it sits in separate rows so filtering/sorting the log never affects it:"))
body.append(formula_table([
    ("A6:A19", "TODAY()", "Date of the interest snapshot (auto)."),
    ("B6:B19", "(text) Interest", "Transaction type."),
    ("E6:E19", "IFERROR(VLOOKUP(D6,Interest!$D$6:$Q$19,13,FALSE),0)",
        "Interest earned to date for the account - the same value folded into the Balance sheet's Current Balance. "
        "It reads from the Interest sheet (based only on principal), so it never double-counts or creates a loop."),
    ("E20", "SUM(E6:E19)", "Total interest earned to date across all accounts."),
]))
