...
body.append(para("The Deposit tab and the LEFT side of the Transactions tab are data-entry tables (Excel Tables with "
                 "filter dropdowns) - you type each entry on a new row. Columns: Deposit = Date, Bank, Account, Amount, "
                 "Notes; Transactions = Date, Type, Bank, Account, Amount (use negatives for money out), Notes."))
body.append(para("Just below the Transaction Log (rows 22-38) is an auto-posted Interest ledger - one line per account, "
                 "read-only, generated by formula (not typed). It sits in separate rows from the log so filtering or "
                 "sorting the log never affects it:"))
body.append(formula_table([
    ("A24:A37", "TODAY()", "Date of the interest snapshot (auto)."),
    ("B24:B37", "(text) Interest", "Transaction type."),
    ("E24:E37", "IFERROR(VLOOKUP(D24,Interest!$D$6:$Q$19,13,FALSE),0)",
    ...
    ("E38", "SUM(E24:E37)", "Total interest earned to date across all accounts."),
]))
