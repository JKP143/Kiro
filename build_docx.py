...
body.append(para("The RIGHT side of the Transactions tab is an auto-posted Interest ledger (one line per account, "
                 "read-only). It is generated by formula, not typed:"))
body.append(formula_table([
    ("H6:H19", "TODAY()", "Date of the interest snapshot (auto)."),
    ("I6:I19", "(text) Interest", "Transaction type."),
    ("L6:L19", "IFERROR(VLOOKUP(K6,Interest!$D$6:$Q$19,13,FALSE),0)",
        "Interest earned to date for the account - the same value folded into the Balance sheet's Current Balance. "
        "It reads from the Interest sheet (which is based only on principal), so it never double-counts or creates a loop."),
    ("L20", "SUM(L6:L19)", "Total interest earned to date across all accounts."),
]))
