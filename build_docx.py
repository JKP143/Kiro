...
body.append(para("The Deposit tab and the Transaction Log (lower part of the Transactions tab) are data-entry tables "
                 "(Excel Tables with filter dropdowns) - you type each entry on a new row. Columns: Deposit = Date, Bank, "
                 "Account, Amount, Notes; Transaction Log = Date, Type, Bank, Account, Amount (use negatives for money "
                 "out), Notes. The Transaction Log has nothing beneath it, so it can grow as long as you need."))
body.append(para("At the TOP of the Transactions tab (rows 4-20) is an auto-posted Interest ledger - one line per "
                 "account, read-only, generated by formula (not typed). It shows each account's interest earned PER DAY. "
                 "It is kept above the log so the log can grow freely, and it sits in separate rows so filtering/sorting "
                 "the log never affects it. (The running total that is actually added to your balance is on the Balance "
                 "and Interest tabs.)"))
body.append(formula_table([
    ("A6:A19", "TODAY()", "Date of the interest snapshot (auto)."),
    ("B6:B19", "(text) Interest", "Transaction type."),
    ("E6:E19", "IFERROR(VLOOKUP(D6,Interest!$D$6:$R$19,6,FALSE),0)",
        "Interest earned per day for the account (Balance x Annual Rate / 365). Pulled from the Interest tab's Interest / Day column."),
    ("E20", "SUM(E6:E19)", "Total interest earned per day across all accounts."),
]))
...
body.append(formula_table([
    ...
    ("E6:E19", "SUMIFS(tblTransactions[Amount],tblTransactions[Account],C6)",
